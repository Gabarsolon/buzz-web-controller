...
not something to hardcode: it's measured here by joining each slot over the
server's own WebSocket, pressing red, and watching pygame (itself just an
SDL client, the same as PCSX2) for the resulting button event.
...
Usage:
    python tools/bind_pcsx2.py [--url ws://127.0.0.1:8420/ws] [--players 4]
...
async def identify_slot(url: str, slot: int, instance_to_index: dict, timeout: float = 2.0):
    """Join `slot`, tap red, return the 0-based SDL device index that reacted.
    ...
    async with websockets.connect(url) as ws:
        ...
        await ws.send(json.dumps({'type': 'join', 'slot': slot}))
        reply = json.loads(await ws.recv())
        if reply.get('type') == 'join_rejected':
            return 'taken'
        if reply.get('type') != 'joined':
            raise RuntimeError(f'slot {slot}: unexpected server reply ({reply})')

        await ws.send(json.dumps({'type': 'down', 'button': 'red'}))
        found = None
        ...
        await ws.send(json.dumps({'type': 'up', 'button': 'red'}))
        return found
...
async def main_async(args):
    ...
    for slot in range(1, args.players + 1):
        idx = await identify_slot(args.url, slot, instance_to_index)
        if idx == 'taken':
            print(f'  player {slot}  ->  SKIPPED (a phone already holds this '
                 f'slot -- close that tab/browser and rerun to bind it)')
            skipped.append(slot)
            continue
        if idx is None:
    ...
    if not mapping:
        sys.exit('\nNo slots could be identified -- nothing to write.')
    ...
    if skipped:
        print(f'Slot(s) {skipped} were skipped (already held) and left unwritten. '
             'Rerun once those phones disconnect to add them.')
...
def main():
    ...
    ap.add_argument('--url', default='ws://127.0.0.1:8420/ws')
